...
### 3. Принять самоподписанный SSL сертификат

⚠️ **ВАЖНО:** Первый раз откройте в браузере:
```
https://37.252.23.30:9696
```
...
После этого API будет доступен по HTTPS без блокировки Mixed Content.
...
2. **Сначала** откройте `https://37.252.23.30:9696` в браузере и примите сертификат
3. В виджете API используйте **PUT** или **POST** запрос на `https://37.252.23.30:9696/api/task`
